...
from farmer import (
    search_farmers, get_farmer, add_farmer, update_farmer, delete_farmer,
)


class FarmerDialog(QDialog):
    def __init__(self, farmer_id: int = 0, parent=None):
        super().__init__(parent)
        self.farmer_id = farmer_id
        self.setWindowTitle("Edit Farmer" if farmer_id else "Add Farmer")
        ...
        self.name_edit.setPlaceholderText("Farmer name *")
        ...
        if farmer_id:
            ...
            if f:
                ...
                self.mobile_edit.setText(f.get("mobile", ""))
                self.address_edit.setText(f.get("address", ""))
    ...
    def _validate(self):
        if not self.name_edit.text().strip():
            QMessageBox.warning(self, "Error", "Farmer name is required.")
            return
        self.accept()
...
class FarmerWindow(QWidget):
    ...
    def _refresh(self):
        ...
        for f in farmers:
            ...
            self.table.setItem(row, 2, QTableWidgetItem(f.get("mobile", "")))
            self.table.setItem(row, 3, QTableWidgetItem(f.get("address", "")))
            self.table.setItem(row, 4, QTableWidgetItem(f.get("created_at", "")[:10]))
            ...
            edit_btn.clicked.connect(lambda checked, fid=fid: self._edit_farmer(fid))
            action_layout.addWidget(edit_btn)

            del_btn = QPushButton("Del")
            del_btn.setFixedSize(50, 26)
            del_btn.setStyleSheet("background: #f44336; color: white; border: none; border-radius: 3px;")
            del_btn.clicked.connect(lambda checked, fid=fid: self._delete_farmer(fid))
            action_layout.addWidget(del_btn)

            self.table.setCellWidget(row, 5, action_w)

    def _add_farmer(self):
        dialog = FarmerDialog(0, self)
        if dialog.exec() == QDialog.Accepted:
            data = dialog.get_data()
            add_farmer(**data)
            self._refresh()

    def _edit_farmer(self, farmer_id: int):
        dialog = FarmerDialog(farmer_id, self)
        if dialog.exec() == QDialog.Accepted:
            data = dialog.get_data()
            update_farmer(farmer_id, **data)
            self._refresh()

    def _delete_farmer(self, farmer_id: int):
        ok = QMessageBox.question(
            self, "Confirm", "Delete this farmer?",
